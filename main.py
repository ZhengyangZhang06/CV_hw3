...
class BoundingBoxesTransform(object):
    ...
    def player2camera(cords, camera):
        ...
        camera_transform = BoundingBoxesTransform._complete_transform(camera.get_transform())
        cords = np.zeros((8, 4))
        rel_x = cords[0, 0] - camera_transform.location.x
        rel_y = cords[0, 1] - camera_transform.location.y
        rel_z = cords[0, 2] - camera_transform.location.z
        camera_transform = BoundingBoxesTransform._complete_transform(camera.get_transform())
        ...
        inverse_transform = np.linalg.inv(final_transform)
        for i in range(8):
            cords[i, 0] = cords[i, 0] + rel_x
            cords[i, 1] = cords[i, 1] + rel_y
            cords[i, 2] = cords[i, 2] + rel_z
            cords[i, 3] = cords[i, 3]
